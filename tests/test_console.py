...
class TestHBNBCommand_help(unittest.TestCase):
    ...
    def test_help_quit(self):
        hq = "Quit command to exit the program."
        with patch("sys.stdout", new=StringIO()) as output:
            self.assertFalse(HBNBCommand().onecmd("help quit"))
            self.assertEqual(hq, output.getvalue().strip())

    def test_help_create(self):
        hc = ("Usage: create <class>\n        "
             "Create a new class instance and print its id.")
        with patch("sys.stdout", new=StringIO()) as output:
            self.assertFalse(HBNBCommand().onecmd("help create"))
            self.assertEqual(hc, output.getvalue().strip())

    def test_help_EOF(self):
        hE = "EOF signal to exit the program."
        with patch("sys.stdout", new=StringIO()) as output:
            self.assertFalse(HBNBCommand().onecmd("help EOF"))
            self.assertEqual(hE, output.getvalue().strip())

    def test_help_show(self):
        hs = ("Usage: show <class> <id> or <class>.show(<id>)\n        "
             "Display the string representation of a class instance of"
             " a given id.")
        with patch("sys.stdout", new=StringIO()) as output:
            self.assertFalse(HBNBCommand().onecmd("help show"))
            self.assertEqual(hs, output.getvalue().strip())

    def test_help_destroy(self):
        hd = ("Usage: destroy <class> <id> or <class>.destroy(<id>)\n        "
             "Delete a class instance of a given id.")
        with patch("sys.stdout", new=StringIO()) as output:
            self.assertFalse(HBNBCommand().onecmd("help destroy"))
            self.assertEqual(hd, output.getvalue().strip())

    def test_help_all(self):
        ha = ("Usage: all or all <class> or <class>.all()\n        "
             "Display string representations of all instances of a given class"
             ".\n        If no class is specified, displays all instantiated "
             "objects.")
        with patch("sys.stdout", new=StringIO()) as output:
            self.assertFalse(HBNBCommand().onecmd("help all"))
            self.assertEqual(ha, output.getvalue().strip())

    def test_help_count(self):
        ht = ("Usage: count <class> or <class>.count()\n        "
             "Retrieve the number of instances of a given class.")
        with patch("sys.stdout", new=StringIO()) as output:
            self.assertFalse(HBNBCommand().onecmd("help count"))
            self.assertEqual(ht, output.getvalue().strip())

    def test_help_update(self):
        hu = ("Usage: update <class> <id> <attribute_name> <attribute_value> or"
        ...
        with patch("sys.stdout", new=StringIO()) as output:
            ...
            self.assertEqual(hu, output.getvalue().strip())
